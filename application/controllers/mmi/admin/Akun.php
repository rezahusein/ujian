...
//============================================================================================================
	...
	public function index()
	{
		...
		$this->template->load('mmi/layouts/app','mmi/admin/akun/user',$this->data);
	}

	public function json()
	{
		header('Content-Type: application/json');
        $this->datatables->select('user.id,user.nip,user.name,role.role,user.desc');
        ...
        if(!empty($_GET['role']) && @$_GET['role'] == 'peserta'){
        	$this->datatables->where('role.role','Peserta');
        }
        else{
        	$this->datatables->where('role.role!=','Peserta');
        }
	...
	public function edit($id)
	{
		...
		$this->load->view('mmi/admin/akun/user-edit',$data);
	}

	public function editUser($id)
	{
		$id = $this->template->sonDecode($id);
		$data['user_data'] = $this->mymodel->selectDataone('user',array('uid' =>$id));
		$data['file'] = $this->mymodel->selectDataone('file',array('table_id'=>$id,'table'=>'user'));
		$data['page_name'] = 'master';
		$data['id'] = $id;

		$this->template->load('mmi/layouts/app','mmi/admin/akun/edit-user',$data);
	}
	...
	public function updateUser()
	{
		...
		if($_FILES['file']['name']!='')
	    {
	        ...
	        if ( ! $this->upload->do_upload('file')){
				...
				// redirect('mmi/admin/akun/editUser/'.$id,'refresh');
			}
		}else{
	...
	public function editUser_redirect($id){
	    ...
	    redirect('mmi/admin/akun/editUser/'.$id);
	}
}
