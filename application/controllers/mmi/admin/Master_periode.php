...
// $this->form_validation->set_rules('dt[created_by]', '<strong>Created By</strong>', 'required');
		...
		public function json()
		{
	        ...
	        $this->datatables->select('master_periode.standar_nilai,master_periode.id,CONCAT(DATE_FORMAT(periode_dari, "%d %M %Y")," - ",DATE_FORMAT(periode_sampai, "%d %M %Y")) AS periode,master_periode.periode_dari,master_periode.periode_sampai,master_periode.lama_waktu_ujian,persentase_pg,master_periode.persentase_essay,master_periode.status,user.name as created_by,(SELECT COUNT(*) FROM master_soal WHERE master_soal.id_periode=master_periode.id) AS jumlah_soal,(SELECT COUNT(*) FROM master_soal WHERE master_soal.id_periode=master_periode.id AND master_soal.jenis_soal="pg") AS jumlah_soal_pg,(SELECT COUNT(*) FROM master_soal WHERE master_soal.id_periode=master_periode.id AND master_soal.jenis_soal="essay") AS jumlah_soal_essay,(SELECT COUNT(*) FROM peserta_periode WHERE peserta_periode.status="ENABLE" AND peserta_periode.id_periode=master_periode.id) AS jumlah_peserta,(SELECT COUNT(*) FROM peserta_periode WHERE peserta_periode.status="ENABLE" AND peserta_periode.id_periode=master_periode.id AND peserta_periode.status_ujian = "belum ujian") AS peserta_belum_ujian,(SELECT COUNT(*) FROM peserta_periode WHERE peserta_periode.status="ENABLE" AND peserta_periode.id_periode=master_periode.id AND peserta_periode.status_ujian = "sedang ujian") AS peserta_sedang_ujian,(SELECT COUNT(*) FROM peserta_periode WHERE peserta_periode.status="ENABLE" AND peserta_periode.id_periode=master_periode.id AND peserta_periode.status_ujian = "menunggu hasil") AS peserta_menunggu_hasil,(SELECT COUNT(*) FROM peserta_periode WHERE peserta_periode.status="ENABLE" AND peserta_periode.id_periode=master_periode.id AND peserta_periode.status_ujian = "selesai ujian") AS peserta_selesai_ujian');
