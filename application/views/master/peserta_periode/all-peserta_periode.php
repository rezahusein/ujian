 ...
 <!-- Content Wrapper. Contains page content -->
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li class="active">Peserta Periode</li>
      </ol>
    </section>
    <!-- Main content -->
            ...
            <div class="box-header">
              <div class="row">
                ...
                <div class="col-md-6">
                  ...
                  <a href="<?= base_url('master/Peserta_periode/create') ?>">
                    <button type="button" class="btn btn-sm btn-success"><i class="fa fa-plus"></i> Tambah Peserta Periode</button> 
                  </a>
                  <a href="<?= base_url('fitur/ekspor/peserta_periode') ?>" target="_blank">
                    <button type="button" class="btn btn-sm btn-warning"><i class="fa fa-file-excel-o"></i> Ekspor Peserta Periode</button> 
                  </a>
                  <button type="button" class="btn btn-sm btn-info" onclick="$('#modal-impor').modal()"><i class="fa fa-file-excel-o"></i> Import Peserta Periode</button>
                  </div>
                </div>  
              </div>
  ...
  <script type="text/javascript">
        ...
        function loadtable(status) {
            var table = '<table class="table table-bordered" id="mytable">'+
             ...
             $("#load-table").html(table)
              ...
              var t = $("#mytable").dataTable({
                ...
                processing: true,
                serverSide: true,
                ajax: {"url": "<?= base_url('master/Peserta_periode/json?status=') ?>"+status, "type": "POST"},
                columns: [
                    ...
                    { targets : [12],
                        render : function (data, type, row, meta) {
                              if(row['status']=='ENABLE'){
                                var htmls = '<a href="<?= base_url('master/Peserta_periode/status/') ?>'+row['id']+'/DISABLE">'+
                                            '    <button type="button" class="btn btn-sm btn-sm btn-success"><i class="fa fa-home"></i> ENABLE</button>'+
                                            '</a>';
                              }else{
                                var htmls = '<a href="<?= base_url('master/Peserta_periode/status/') ?>'+row['id']+'/ENABLE">'+
         ...
         function edit(id) {
            location.href = "<?= base_url('master/Peserta_periode/edit/') ?>"+id;
         }
